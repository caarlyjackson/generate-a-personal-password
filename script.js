...
function generatePassword() {
  ...
  var specialCharactersAnswer = specialCharacters;
  if (specialCharactersAnswer === true) {
    allOptions.push(specialCharacters);
  }
  // Uppercase True
  var uppercaseAnswer = upperCase;
  if (uppercaseAnswer === true) {
    allOptions.push(upperCase.length);
  }
  // Lowercase True
  var lowercaseAnswer = lowerCase;
  if (lowercaseAnswer === true) {
    allOptions.push(lowerCase.length);
  }
  // Numerical True
  var numericalAnswer = numerical;
  if (numericalAnswer === true) {
    allOptions.push(numerical.length);
  }
  return passwordInfo;
};
...
for (i = 0; i < sizeOfPassword; i++) {
  ...
  var password = []
  var randomCharacter = allOptions[""];
